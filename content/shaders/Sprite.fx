...
GSPS_INPUT VS_Main( VS_Input vertex ) {
    ...
    vsOut.Pos = vertex.pos;//mul( vertex.pos, mvp_ );
...
[maxvertexcount(4)]
void GS_Main(point GSPS_INPUT gin[1], uint primID : SV_PrimitiveID, inout TriangleStream<PS_Input> triStream)
{    
    ...
    for(int i = 0; i < 4; ++i) {
        ...
        xt += pos.x;
        yt += pos.y;
        gout.pos = mul(float4(xt, yt, 0.0f, 1.0f), mvp_); 
        gout.tex0 = t[i];
